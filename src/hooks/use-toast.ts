  ...
  switch (action.type) {
    ...
    case "DISMISS_TOAST": {
      ...
      if (toastId) {
        ...
        addToRemoveQueue(toastId, t?.action ? TOAST_REMOVE_DELAY : ((t as any)?.duration || 5000))
      } else {
        state.toasts.forEach(t => addToRemoveQueue(t.id, t.action ? TOAST_REMOVE_DELAY : ((t as any)?.duration || 5000)))
      }
